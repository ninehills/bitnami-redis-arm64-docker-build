...
ARM64_DOWNLOAD="https://github.com/oliver006/redis_exporter/releases/download/v1.26.0/redis_exporter-v1.26.0.linux-arm64.tar.gz"

[[ -d output/ ]] && rm -r output/
mkdir -p output/
cd output/

wget "${ARM64_DOWNLOAD}" -O redis_exporter-v1.26.0.linux-arm64.tar.gz
tar zxf redis_exporter-v1.26.0.linux-arm64.tar.gz

wget "${BITNAMI_DOWNLOAD_URL}/${BITNAMI_PACKAGE}.tar.gz" -O ${BITNAMI_PACKAGE}.tar.gz
tar zxf ${BITNAMI_PACKAGE}.tar.gz

cp redis_exporter-v1.26.0.linux-arm64/redis_exporter ${BITNAMI_PACKAGE}/files/redis-exporter/bin
